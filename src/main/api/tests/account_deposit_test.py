...
class TestDepositAccount:
    ...
    def test_deposit_valid(self, db_session: Session, api_manager: ApiManager, create_user_request: CreateUserRequest,
                           create_account_request_role_user, amount: int | float):
        ...
        assert data["id"] == payload.accountId, 'ID аккаунта не совпадает'
        assert data["balance"] == payload.amount, 'Баланс после пополнения не совпадает'

        transaction_response = api_manager.user_steps.get_account_transaction(create_user_request, response.id)
        assert transaction_response.transactions[0].amount == payload.amount, 'Сумма в транзакции не совпадает'
